...
namespace KvizAPI.Services
{
    public class UserService : IUserService
    {
        ...
        {
            var quiz = _mapper.Map<Entities.Quiz>(quizDto);
            var user = await GetOriginalCurrentUser();
            quiz.CreatorId = user.Id;

            for (int i = 0; i < quizDto.Questions.Count(); i++)
                quiz.Questions.ElementAt(i).Options = _mapper.Map<IEnumerable<Entities.Answer>>(quizDto.Questions[i].Options);
